...
from myarm_m750_core.domain.models import CommandContext, JointTarget
...
def test_mock_adapter_updates_measured_state() -> None:
    ...
    assert adapter.write_joint_target(target, context).succeeded
    assert adapter.read_joint_state(context).position_rad == target.position_rad
    adapter.disconnect()
...
def test_replay_adapter_is_read_only(tmp_path: Path) -> None:
    ...
    adapter.connect()
    context = CommandContext.with_timeout(1.0)
    assert adapter.read_joint_state(context).sequence == 1
    assert not adapter.write_joint_target(JointTarget((0.0,) * 6), context).succeeded
